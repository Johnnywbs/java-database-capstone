...
- **location_id**: INT, Foreign Key -> clinic_locations(id)  <-- ADICIONE ISSO
...
- **amount**: DECIMAL(10, 2), Not Null (Use DECIMAL for currency to avoid rounding errors)
...
Stores insurance details for patients to process coverage.
- **id**: INT, Primary Key, Auto Increment
...
Stores medical prescriptions issued after an appointment. Using MongoDB allows for flexibility (e.g., varying number of medications, nested pharmacy details).
